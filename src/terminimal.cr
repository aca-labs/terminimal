...
module Terminimal
  ...
  # ditto
  def spinner(await : Future, style = Spinner::Style::UNI_DOTS, async = false, &message : Proc(String))
  ...
  # ditto
  def spinner(await : Future, style = Spinner::Style::UNI_DOTS, async = false, message = "")
    spinner(await, style, async) { message }
  end
